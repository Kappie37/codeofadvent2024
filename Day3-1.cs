namespace AdventOfCode2024
{
    internal class Day3_1
    {
        ...
        const string simple_input = "xmul(2,4)%&mul[3,7]!@^do_not_mul(5,5)+mul(32,64]then(mul(11,8)mul(8,5))";

        string input => large_input;

        readonly string numbers = "0123456789";
        int currentCheckStage = 0;

        string firstNumber = string.Empty;
        string secondNumber = string.Empty;
        int sum = 0;

        readonly char[] stages = new char[] { 'm', 'u', 'l', '(', 'x', ',', 'y', ')' };
        ...
        public object GetAnswer()
        {
            while (index < input.Length)
            {
                ...
                switch (currentCheckStage)
                {
                    ...
                    case 5:
                    case 7:
                        HandleStage(currentCharacter, currentCheckStage);
                        break;
                    case 4:
                        HandleNumber(currentCharacter, ref firstNumber);
                        break;
                    case 6:
                        HandleNumber(currentCharacter, ref secondNumber);
                        break;
                }
                index++;
            }
        ...
        private void HandleNumber(char currentCharacter, ref string number)
        {
            if (numbers.Contains(currentCharacter))
            {
                if (index + 1 < input.Length)
                {
                    ...
                    if (numbers.Contains(nextCharacter))
                    {
                        if (index + 2 < input.Length)
                        {
                            var secondNextCharacter = input[index + 2];
                            if (numbers.Contains(secondNextCharacter))
                            {
                                number = currentCharacter.ToString() + nextCharacter.ToString() + secondNextCharacter.ToString();
                                index += 2;
                            }
                            else
                            {
                                number = currentCharacter.ToString() + nextCharacter.ToString();
                                index++;
                            }
                        }
                    }
                    ...
                    {
                        number = currentCharacter.ToString();
                    }
                }
        ...
        private void HandleStage(char currentCharacter, int v)
        {
            if (currentCharacter == stages[v])
            {
                if (currentCheckStage == 7)
                {
                    AddNumber();
                }
                else
