...
namespace AdventOfCode2024
{
    internal class Day3_2
    {
        ...
        public class MulChecker(string inputString)
        {
            ...
            public void Check(char character)
            {
                switch (CurrentStage)
                {
                    ...
                    case 5:
                    case 7:
                        ValidateForCurrentStage(character);
                        break;
                    case 4:
                        FirstNumber = ValidateNumber(character);
                        break;
                    case 6:
                        SecondNumber = ValidateNumber(character);
                        break;
                }
            }

            private int ValidateNumber(char character)
            {
                var number = string.Empty;
                if (allNumbers.Contains(character))
                {
                    if (CurrentIndex + 1 < InputString.Length)
                    {
                        var nextCharacter = InputString[CurrentIndex + 1];
                        if (allNumbers.Contains(nextCharacter))
                        {
                            if (CurrentIndex + 2 < InputString.Length)
                            {
                                var secondNextCharacter = InputString[CurrentIndex + 2];
                                if (allNumbers.Contains(secondNextCharacter))
                                {
                                    number = character.ToString() + nextCharacter.ToString() + secondNextCharacter.ToString();
                                    CurrentIndex += 2;
                                }
                                else
                                {
                                    number = character.ToString() + nextCharacter.ToString();
                                    CurrentIndex++;
                                }
                            }
                        }
            ...
            private void ValidateForCurrentStage(char character)
            {
                if (character == Stages[CurrentStage])
                {
                    if (CurrentStage == 7)
                    {
                        AddNumber();
        ...
        public class DoDontChecker()
        {
            public char[] DoStages { get; set; } = new char[] { 'd', 'o', '(', ')' };
            public char[] DontStages { get; set; } = new char[] { 'd', 'o', 'n', '\'', 't', '(', ')' };
            public int CurrentStage { get; set; }
            ...
            {
                if (CurrentStage < DoStages.Length && character == DoStages[CurrentStage])
                {
                    CurrentStage++;
                ...
                else if (CurrentStage < DontStages.Length && character == DontStages[CurrentStage])
                {
                    CurrentStage++;

                    if (CurrentStage == DontStages.Length)
                    {
                        Do = false;
                        Reset();
                    }
